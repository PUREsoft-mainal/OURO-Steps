...
io.on('connection', (socket) => {
    ...
    socket.on('transfer_ouro_coins', async (payload) => {
        ...
        // ==========================================================================
    // 🏫 💬 [تم الحقن موضعياً] - مستمع بث وتمرير تعليقات الطلاب داخل السنتر حياً
    // ==========================================================================
    socket.on('send_center_live_comment', (commentPayload) => {
        try {
            if (!commentPayload || !commentPayload.text) return;
            // إعادة توجيه وبث التعليق لجميع المتصلين بالغرفة التعليمية حالياً
            socket.broadcast.emit('receive_center_live_comment', commentPayload);
        } catch (err) { console.error("خطأ تمرير بث تعليقات السنتر:", err); }
    });
